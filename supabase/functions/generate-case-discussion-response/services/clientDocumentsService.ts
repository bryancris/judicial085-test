...
export async function fetchClientDocuments(clientId: string, caseId: string | null = null) {
  ...
  try {
    ...
    // Build query URL with filters - limit to 50 chunks to prevent context overflow
    let queryUrl = `${supabaseUrl}/rest/v1/document_chunks?select=document_id,content,metadata,chunk_index&client_id=eq.${clientId}&order=document_id.asc,chunk_index.asc&limit=50`;
    ...
    // Format documents for context inclusion - limit to top 10 documents
    return Array.from(documentMap.values())
      ...
      .map(doc => {
        ...
        // Limit content to a reasonable size for context (1500 chars per document)
        const snippet = doc.content.length > 1500 
