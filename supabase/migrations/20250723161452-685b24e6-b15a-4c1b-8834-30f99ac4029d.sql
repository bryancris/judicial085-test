-- Step 5: Database Integration for Case Discussion Research (Fixed)

-- Add foreign key to link perplexity research to specific case discussion messages
ALTER TABLE perplexity_research 
ADD COLUMN IF NOT EXISTS case_discussion_id uuid REFERENCES case_discussions(id) ON DELETE SET NULL;

-- Add indexes for faster lookups
CREATE INDEX IF NOT EXISTS idx_perplexity_research_case_discussion_id ON perplexity_research(case_discussion_id);
CREATE INDEX IF NOT EXISTS idx_perplexity_research_client_search ON perplexity_research(client_id, search_type, created_at DESC);
