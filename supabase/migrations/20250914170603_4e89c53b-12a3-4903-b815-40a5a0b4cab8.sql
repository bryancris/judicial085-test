-- Create function to extract timeline events from existing medical analyses
CREATE OR REPLACE FUNCTION extract_timeline_events_from_medical_analyses()
RETURNS TABLE(extracted_count integer, client_id uuid)
LANGUAGE plpgsql
AS $$
DECLARE
    rec RECORD;
...
BEGIN
    ...
    LOOP
        ...
        LOOP
            ...
            ON CONFLICT (client_id, event_date, event_type, description) DO NOTHING;
            
            extracted_count := extracted_count + 1;
        END LOOP;
    END LOOP;
    
    RETURN QUERY SELECT extracted_count, rec.client_id;
END;
$$;

-- Execute the function to migrate existing data
SELECT extract_timeline_events_from_medical_analyses();
